...
def add_self_loops_indices(indices, n_nodes=None):
    ...
    mask = tf.map_fn(lambda i: i[0] != i[1], indices, tf.bool)
    sl_indices = tf.range(n_nodes, dtype=indices.dtype)[:, None]
    sl_indices = tf.repeat(sl_indices, 2, -1)
    indices = tf.concat((tf.boolean_mask(indices, mask), sl_indices), 0)
